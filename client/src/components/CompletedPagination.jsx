...
import { getNovelsCompleted } from '../action/novelAction';

const CompletedPagination = ({ total, pagination }) => {
  ...
  const handlePageChange = (changePage) => {
    if (changePage < 1) {
      setCurrentPage(1);
      dispatch(getNovelsCompleted(1));
    }
    if (changePage > pages) {
      setCurrentPage(pages);
      dispatch(getNovelsCompleted(pages));
    }
    if (changePage > 0 && changePage <= pages) {
      setCurrentPage(changePage);
      dispatch(getNovelsCompleted(changePage));
    }
  };

...
const ChapterListPagination = ({ total, pagination }) => {
  ...
  const handlePageChange = (changePage) => {
    if (changePage < 1) {
      setCurrentPage(1);
      dispatch(getAllChapters(1));
    }
    if (changePage > pages) {
      setCurrentPage(pages);
      dispatch(getAllChapters(pages));
    }
    if (changePage > 0 && changePage <= pages) {
      setCurrentPage(changePage);
      dispatch(getAllChapters(changePage));
    }
  };
...
export default ChapterListPagination;
